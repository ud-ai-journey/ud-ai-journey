...
def analyze_session_data(log_data: List[Dict]) -> Optional[Dict]:
    ...
    # Calculate average ratings per mood and process feedback comments
    avg_ratings = {}
    feedback_by_mood = {}
    
    for mood in set(mood_usage.keys()).union(set(mood_ratings.keys())):
        # Calculate average rating
        ratings = mood_ratings.get(mood, [])
        if ratings:
            avg_ratings[mood] = sum(ratings) / len(ratings)
        
        # Process feedback for this mood
        mood_feedback = [f for f in feedback_data if f['mood'] == mood]
        if mood_feedback:
            feedback_by_mood[mood] = {
                'average_rating': sum(f['rating'] for f in mood_feedback) / len(mood_feedback),
                'count': len(mood_feedback),
                'recent_ratings': [f['rating'] for f in mood_feedback[-5:]],  # Last 5 ratings
                'positive_keywords': [],  # Will be populated based on comments
                'negative_keywords': []   # Will be populated based on comments
            }
    ...
    analysis = {
        ...
        'feedback_analysis': {
            ...
            'feedback_by_mood': feedback_by_mood
        },
